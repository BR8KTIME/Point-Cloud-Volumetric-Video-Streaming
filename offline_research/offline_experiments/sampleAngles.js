...
import { modifyShader } from '/customshader.js'
import { deltaE } from './LabSpace.js';
import { euclideanDistance3D, euclideanDistance2D, median, average, computeIQR, dot, normalize, ORMatrices, checkMatrices, standard_deviation, getModelViewMatrix } from './utility.js';
import { remove_pointsNormalBased, remove_pointsSignBased, getSigns} from './removePoints3D.js';
import fs from "fs";
import { concave } from './hull.js';
import { saveFile , readZipFromArrayBuffer, loadSigns, prepareFileForDownload} from './io.js';
...
async function init() {
	...
	currentAngle = 60;    // 0 = front, 90 = right, 180 = back, etc.
...
async function start() {
    let baseUrl = 'http://192.168.125.132/dataset/longdress/normalSequences/';
    let level = 'level';
    let pcdFormat = '.pcd';
    ...
    for (let i = 0; i < totalFrames; i++) {
        ...
        for (let j = 0; j < 10; j++) {
            ...
            allLoadingPromises.push(loadPointCloud(newURL, fileName));
        }
    }
