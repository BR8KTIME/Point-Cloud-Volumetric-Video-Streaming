...
async function modify_pixels(level) {
	...
	try
	{
		...
		// 1. Color Texture (Base Image)
		const colorTexture = new THREE.DataTexture(pixels, width, height, THREE.RGBAFormat);
		colorTexture.needsUpdate = true;

		// 2. Mask Texture (Hole Detection Output)
		// Using RedFormat because the mask only requires a single channel (0 or 1)
		const maskTexture = new THREE.DataTexture(holeArray, width, height, THREE.RedFormat);
		maskTexture.needsUpdate = true;

		console.time("Hole Concealment Time");
		// 3. Define the Shader Material
		const interpolationMaterial = new THREE.ShaderMaterial({
			uniforms: {
			...
			fragmentShader: `
				...
				void main() {
					vec4 centerColor = texture(tColor, vUv);
					float isHole = texture(tMask, vUv).r;

					// If not a hole, bypass computation
					if (isHole < 0.5) {
					...
					vec3 sumWeightedColor = vec3(0.0);
					float sumWeight = 0.0;
					float validCount = 0.0;
					
					// Calculate interval dynamically using uniform integer math
					int interval = kernelSize / 2;

					for (int i = -interval; i <= interval; i++) {
						for (int j = -interval; j <= interval; j++) {
							if (i == 0 && j == 0) continue;

							vec2 offset = vec2(float(j), float(i)) / resolution;
							vec2 neighborUv = vUv + offset;
							
							// Sample neighbor mask array
							float neighborIsHole = texture(tMask, neighborUv).r;

							// Exact match: if(holeArray[holeIdx] != 1)
							if (neighborIsHole < 0.5) {
								vec4 neighborColor = texture(tColor, neighborUv);
								
								float dist = length(vec2(float(j), float(i)));
								float weight = 1.0 / (dist * dist);
								
								sumWeightedColor += neighborColor.rgb * weight;
								sumWeight += weight;
								validCount += 1.0;
							}
						}
					...
					float threshold = float(kernelSize * kernelSize) * 0.3;
					
					if (validCount > threshold) {
						// Apply weighted RGB values. Assign alpha 1.0 assuming concealed pixels should be fully opaque.
						gl_FragColor = vec4(sumWeightedColor / sumWeight, 1.0);
					} else {
						// Threshold failed; retain original pixel configuration
						gl_FragColor = centerColor; 
						gl_FragColor.a = 1.0;
					}
				}
		...
		console.timeEnd("Hole Concealment Time");

		console.time("Upload Mesh Time");
		const geometry = new THREE.PlaneGeometry(width, height);
		modifiedMesh = new THREE.Mesh(geometry, interpolationMaterial);
		modifiedMesh.position.set(width / 2, height / 2, 0);

		// Setup Orthographic Camera
		let textureCamera = new THREE.OrthographicCamera(0, width, height, 0, -1, 1);
