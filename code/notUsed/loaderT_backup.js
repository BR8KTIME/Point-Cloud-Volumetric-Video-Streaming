...
function loadDrcLEVEL(url) {
	...
	dracoLoader.load(url, function (geometry) {
		...
		level = level + 1;
		loadDrcLEVEL(framebyframe[level]);
	});	
}
...
function takeScreenshot(screenshotLevel) {
  ...
  link.href = dataURL;
//   link.download = `LOD${screenshotLevel + 1}_${pointSize}.png`; 
...
function renderT(screenshotLevel) {
	if (modifiedMesh) {
